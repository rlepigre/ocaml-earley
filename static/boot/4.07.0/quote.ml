...
let dummy_ppat =
  (Helper.Pat.var (let open Location in mkloc "$Antiquotation$" none)).ppat_desc
...
let dummy_psig =
  Psig_open
    {
      popen_lid =
        (let open Location in mkloc (Lident "$Antiquotation$") none);
...
let quote_int : int quote =
  fun _ -> fun loc -> fun i -> Helper.Exp.constant ~loc (Helper.Const.int i)
let quote_int32 : int32 quote =
  fun _ ->
    fun loc ->
      fun i -> Helper.Exp.constant ~loc (Helper.Const.int32 ~suffix:'l' i)
let quote_int64 : int64 quote =
  fun _ ->
    fun loc ->
      fun i -> Helper.Exp.constant ~loc (Helper.Const.int64 ~suffix:'L' i)
let quote_nativeint : nativeint quote =
  fun _ ->
    fun loc ->
      fun i ->
        Helper.Exp.constant ~loc (Helper.Const.nativeint ~suffix:'n' i)
let quote_char : char quote =
  fun _ -> fun loc -> fun c -> Helper.Exp.constant ~loc (Helper.Const.char c)
let quote_string : string quote =
  fun _ ->
    fun loc ->
      fun s ->
        try Hashtbl.find string_anti_table s
        with | Not_found -> Helper.Exp.constant ~loc (Helper.Const.string s)
let quote_bool : bool quote =
  fun _ ->
    fun loc ->
      fun b ->
...
let quote_list : 'a quote -> 'a list quote =
  fun qe ->
    fun e_loc ->
      fun _loc ->
        fun el ->
          let exp_list loc es =
            let open Helper in
              ...
              let cns hd tl =
                ...
                Exp.construct ~loc (Location.mkloc (Lident "::") loc)
                  (Some arg) in
              List.fold_right cns es nil in
          match is_list_antiquotation el with
          | None -> exp_list _loc (List.map (qe e_loc _loc) el)
          | Some (loc, qt) ->
              (try Hashtbl.find anti_table loc qt
               with
               | Not_found -> failwith "antiquotation not in a quotation")
let quote_tuple : expression list quote =
  fun _ -> fun loc -> fun l -> Helper.Exp.tuple ~loc l
let quote_record : (Longident.t * expression) list quote =
  fun _ ->
    fun loc ->
      fun l ->
        let fn (id, e) = ((Location.mkloc id loc), e) in
        Helper.Exp.record ~loc (List.map fn l) None
let quote_apply : (Longident.t * expression list) quote =
  fun _ ->
    fun loc ->
      fun (s, l) ->
...
let quote_const : (Longident.t * expression list) quote =
  fun _ ->
    fun loc ->
      fun (s, l) ->
        let arg =
          ...
          | [] -> None
          | x::[] -> Some x
          | l -> Some (Helper.Exp.tuple ~loc l) in
        Helper.Exp.construct ~loc (Location.mkloc s loc) arg
let quote_option : 'a quote -> 'a option quote =
  fun qe ->
    fun e_loc ->
      fun loc ->
        fun eo ->
          match eo with
          | None ->
              let none = Location.mkloc (Lident "None") loc in
              Helper.Exp.construct ~loc none None
          | Some e ->
              let some = Location.mkloc (Lident "Some") loc in
              Helper.Exp.construct ~loc some (Some (qe e_loc loc e))
let rec quote_longident : Longident.t quote =
  fun e_loc ->
    fun loc ->
      fun l ->
        match l with
        ...
        | Ldot (l, s) ->
            let l = quote_longident e_loc loc l in
            let s = quote_string e_loc loc s in
            quote_const e_loc loc ((longident "Ldot"), [l; s])
        | Lapply (l1, l2) ->
            let l1 = quote_longident e_loc loc l1 in
            let l2 = quote_longident e_loc loc l2 in
            quote_const e_loc loc ((longident "Lapply"), [l1; l2])
let quote_position : Lexing.position quote =
  fun e_loc ->
    fun loc ->
      fun pos ->
        quote_record e_loc loc
          [((lexing "pos_fname"),
             (quote_string e_loc loc pos.Lexing.pos_fname));
...
let quote_location_t : Location.t quote =
  fun e_loc ->
    fun loc ->
      fun l ->
        ...
        then
          quote_record e_loc loc
            [((location "loc_start"),
               (quote_position e_loc loc l.Location.loc_start));
            ((location "loc_end"),
              (quote_position e_loc loc l.Location.loc_end));
            ((location "loc_ghost"),
              (quote_bool e_loc loc l.Location.loc_ghost))]
        else e_loc
(* asttypes.mli *)
