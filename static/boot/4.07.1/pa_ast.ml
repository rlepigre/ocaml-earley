...
open Parsetree
open Longident
let loc_expr ?(attributes= [])  loc e =
  { pexp_desc = e; pexp_loc = loc; pexp_attributes = attributes }
let loc_pat ?(attributes= [])  loc pat =
  { ppat_desc = pat; ppat_loc = loc; ppat_attributes = attributes }
let loc_typ ?(attributes= [])  loc typ =
  { ptyp_desc = typ; ptyp_loc = loc; ptyp_attributes = attributes }
let loc_pcf ?(attributes= [])  loc desc =
  { pcf_desc = desc; pcf_loc = loc; pcf_attributes = attributes }
let merge2 l1 l2 =
...
let rec merge ls =
  ...
  | l1::_ ->
      let rec fn =
        ...
        | l::[] -> l
        | l2::ls ->
            if let open Location in l2.loc_start = l2.loc_end
            then fn ls
            else merge2 l1 l2 in
      fn (List.rev ls)
