  ...
  struct
    ...
    let _ =
      Earley.set_grammar method_type
        (Earley.alternatives
           [Earley.fsequence (typexpr_lvl (next_type_prio DashType))
              (Earley.empty (fun ty -> Oinherit ty));
           Earley.fsequence method_name
             (Earley.fsequence_ignore (Earley.char ':' ':')
                (Earley.fsequence poly_typexpr
                   (Earley.empty (fun pte -> fun mn -> Otag (mn, [], pte)))))])
    ...
    let _ =
      Earley.set_grammar tag_spec
        (Earley.alternatives
           ...
           Earley.fsequence_position tag_name
             (Earley.fsequence
                (Earley.option None
                   (Earley.apply (fun x -> Some x)
                                   ...
                                   fun op ->
                                     ...
                                     fun e' ->
                                       ((next_exp lvl), false,
                                         (fun e ->
                                            fun (_loc, _) ->
                                              mk_binary_op _loc e' op _loc_op
                                                e)))))
        else
          Earley.fsequence
            (Earley.apply List.rev
                               ...
                               fun (_loc, _) ->
                                 List.fold_right
                                   (fun (_loc_e, e', op, _loc_op) ->
                                      fun acc ->
                                        mk_binary_op (merge2 _loc_e _loc) e'
                                          op _loc_op acc) ls e))))
    let _ =
    ...
    let _ =
      semicol__set__grammar
        ([(((fun (alm, lvl) -> lvl > Seq)),
         ...
         (((fun (alm, lvl) -> lvl = Seq)),
           (Earley.fsequence semi_col (Earley.empty (fun _default_0 -> true))));
         (((fun (alm, lvl) -> lvl = Seq)),
           (Earley.fsequence no_semi (Earley.empty (fun _default_0 -> false))))],
          (fun (alm, lvl) -> []))
