  ...
  struct
    ...
    let _ =
      Earley.set_grammar glr_parser
        (Earley.sequence parser_kw glr_rules (fun _default_0  p  -> p))
              ...
              (fun _default_0  l  _default_1  __loc__start__buf 
                 ...
                 let rec fn =
                   ...
                   | (name,arg,ty,r)::l ->
                       ...
                       let pat_name =
                         loc_pat _loc (Ppat_var (id_loc name _loc)) in
                       let pname =
                         match (ty, arg) with
                         | (None ,_) -> pat_name
                         | (Some ty,None ) ->
                             ...
                             loc_pat _loc
                               (Ppat_constraint (pat_name, ptyp_ty))
                         | (Some ty,Some _) ->
                             let ptyp_ty = loc_typ _loc (Ptyp_poly ([], ty)) in
                             loc_pat _loc
                               (Ppat_constraint
                       ...
                       (match arg with
                        | None  ->
                            ...
                            let strname =
                              loc_expr _loc
                                (Pexp_constant (const_string name)) in
                            let name =
                              loc_expr _loc
                                (Pexp_ident (id_loc (Lident name) _loc)) in
                            let e =
                              loc_expr _loc
                                (Pexp_apply (ddg, [(nolabel, strname)])) in
                            let l = value_binding ~attributes:[] _loc pname e in
                            let dsg = exp_glr_fun _loc "set_grammar" in
                            let ev =
                              loc_expr _loc
                                (Pexp_apply
                                   (dsg, [(nolabel, name); (nolabel, r)])) in
                            (((loc_str _loc (Pstr_value (Nonrecursive, [l])))
                              :: str1), ((loc_str _loc (pstr_eval ev)) ::
                              str2))
                        | Some arg ->
                            ...
                            let sname =
                              loc_expr _loc
                                (Pexp_ident (id_loc (Lident set_name) _loc)) in
                            let psname =
                              loc_pat _loc (Ppat_var (id_loc set_name _loc)) in
                            let ptuple =
                              loc_pat _loc (Ppat_tuple [pname; psname]) in
                            let e =
                              loc_expr _loc
                                (Pexp_apply (dgf, [(nolabel, strname)])) in
                            let l =
                              value_binding ~attributes:[] _loc ptuple e in
                            let fam =
                              loc_expr _loc
                                (pexp_fun (nolabel, None, arg, r)) in
                            let ev =
                              loc_expr _loc
                                (Pexp_apply (sname, [(nolabel, fam)])) in
                            (((loc_str _loc (Pstr_value (Nonrecursive, [l])))
                              :: str1), ((loc_str _loc (pstr_eval ev)) ::
                              str2))) in
        ...
        (Earley.alternatives
           [Earley.fsequence (Earley.char '*' '*')
              (Earley.sequence glr_opt_expr
                 ...
                 (fun e  g  _  -> `Fixpoint (e, g)));
           Earley.fsequence (Earley.char '+' '+')
             (Earley.sequence glr_opt_expr
                (Earley.option None
                   (Earley.apply (fun x  -> Some x) (Earley.char '$' '$')))
                (fun e  g  _  -> `Fixpoint1 (e, g)));
           Earley.fsequence (Earley.char '?' '?')
             (Earley.sequence glr_opt_expr
                (Earley.option None
                   (Earley.apply (fun x  -> Some x) (Earley.char '$' '$')))
      ...
      Earley.set_grammar glr_sequence
        (Earley.alternatives
           ...
           Earley.apply_position
             (fun _  __loc__start__buf  __loc__start__pos  __loc__end__buf 
                ...
                let _loc =
                  locate __loc__start__buf __loc__start__pos __loc__end__buf
                    __loc__end__pos in
                (true, (exp_glr_fun _loc "any"))) (Earley.string "ANY" "ANY");
           ...
           Earley.sequence_position
             (Earley.apply_position
                (fun x  str  pos  str'  pos'  ->
                   ((locate str pos str' pos'), x)) char_litteral)
             glr_opt_expr
             (fun c  ->
                let (_loc_c,c) = c in
                fun opt  __loc__start__buf  __loc__start__pos 
                  __loc__end__buf  __loc__end__pos  ->
                  let _loc =
                    locate __loc__start__buf __loc__start__pos
                      __loc__end__buf __loc__end__pos in
           ...
           Earley.sequence_position
             (Earley.apply_position
                (fun x  str  pos  str'  pos'  ->
                   ((locate str pos str' pos'), x)) string_litteral)
             glr_opt_expr
             (fun s  ->
                let (_loc_s,s) = s in
                fun opt  __loc__start__buf  __loc__start__pos 
             ...
             (Earley.sequence (expression_lvl (NoMatch, (next_exp App)))
                ...
                (fun e  opt  _  __loc__start__buf  __loc__start__pos 
                   ...
                   | Pexp_ident { txt = Lident id } ->
                       ...
                       (true,
                         (exp_lab_apply _loc (exp_glrstr_fun _loc "regexp")
                            [((labelled "name"), (exp_string _loc id));
                            (nolabel, e);
                            (nolabel, (exp_fun _loc "groupe" opt))]))
                   | _ ->
                       (true,
                         (exp_apply _loc (exp_glrstr_fun _loc "regexp")
           ...
           Earley.sequence_position
             (Earley.apply_position
                (fun x  str  pos  str'  pos'  ->
                   ((locate str pos str' pos'), x)) regexp_litteral)
             (Earley.apply_position
                (fun x  str  pos  str'  pos'  ->
                   ((locate str pos str' pos'), x)) glr_opt_expr)
             (fun s  ->
                let (_loc_s,s) = s in
                fun opt  ->
    ...
    let _ =
      Earley.set_grammar glr_let
        (Earley.alternatives
              ...
              (Earley.fsequence rec_flag
                 (Earley.fsequence let_binding
                    (Earley.sequence in_kw glr_let
                       (fun _default_0  l  lbs  r  _default_1 
           ...
           Earley.apply (fun _  x  -> x) (Earley.empty ())])
    let glr_cond = Earley.declare_grammar "glr_cond"
    let _ =
      Earley.set_grammar glr_cond
        (Earley.alternatives
           [Earley.sequence when_kw expression (fun _default_0  e  -> Some e);
           ...
           Earley.alternatives
             ...
             Earley.fsequence arrow_re
               (Earley.sequence
                  ...
                  no_semi
                  (fun action  _default_0  _default_1  -> Normal action));
           ...
           Earley.fsequence_position glr_let
             (Earley.fsequence glr_left_member
                (Earley.sequence glr_cond (glr_action alm)
                   (fun condition  action  l  def  __loc__start__buf 
                      __loc__start__pos  __loc__end__buf  __loc__end__pos  ->
                      let _loc =
                        locate __loc__start__buf __loc__start__pos
                          __loc__end__buf __loc__end__pos in
                        ...
                        fst
                          (List.fold_right
                             (fun x  (res,i)  ->
                                ...
                                | `Normal (("_",a),true ,c,d) ->
                                    (((`Normal
                                         ((("_default_" ^ (string_of_int i)),
                                            a), true, c, d, false)) :: res),
                                      (i + 1))
                                | `Normal (id,b,c,d) ->
                                    let occur_loc_id =
                                      ((fst id) <> "_") &&
                          ...
                          (Pexp_let
                             (Nonrecursive,
                               ...
                               (loc_expr _loc
                                  (Pexp_ifthenelse
                                     (c,
                                       (exp_Cons _loc e (exp_ident _loc "y")),
                                       (Some (exp_ident _loc "y"))))))))) ls
              (exp_Nil _loc) in
                 ...
                 (Earley.fixpoint []
                    (Earley.apply (fun x  y  -> x :: y)
                       (Earley.sequence (glr_rule false)
                          (Earley.char '|' '|') (fun r  _  -> r)))))
              (glr_rule true)
              (fun rs  r  _default_0  __loc__start__buf  __loc__start__pos 
                 __loc__end__buf  __loc__end__pos  ->
                 let _loc =
                   locate __loc__start__buf __loc__start__pos __loc__end__buf
                     __loc__end__pos in
