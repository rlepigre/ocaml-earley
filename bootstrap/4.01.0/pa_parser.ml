  ...
  struct
    ...
    let _ =
      Decap.set_grammar glr_parser
        (Decap.sequence parser_kw glr_rules (fun _default_0  p  -> p))
           ...
           (Decap.fsequence glr_ident
              (Decap.sequence glr_sequence glr_option
                 (fun ((cst,s) as _default_0)  opt  ((cst',id) as _default_1)
                     ->
                    `Normal (id, (fopt cst' ((opt <> `Once) || cst)), s, opt))))
    ...
    let _ =
      Decap.set_grammar glr_cond
        (Decap.alternatives
           [Decap.sequence when_kw expression (fun _default_0  e  -> Some e);
