...
and iter_constructor_declaration = fun r1 -> () ; (iter_loc (fun _ -> ())) r1.pcd_name  ; (iter_list iter_core_type) r1.pcd_args  ; (iter_option iter_core_type) r1.pcd_res  ; (fun _ -> ()) r1.pcd_loc  ; iter_attributes r1.pcd_attributes 
...
and iter_extension_constructor_kind c1 =
  match c1 with
  | Pext_decl(x1,x2) -> () ; ((iter_list iter_core_type) x1) ; ((iter_option iter_core_type) x2)  | Pext_rebind(x) -> (iter_loc iter_longident) x
...
and iter_class_type_desc c1 =
  ...
  | Pcty_arrow(x1,x2,x3) -> () ; (iter_label x1) ; (iter_core_type x2) ; (iter_class_type x3)  | Pcty_extension(x) -> iter_extension x
...
and iter_signature_item_desc c1 =
  ...
  | Psig_type(x) -> (iter_list iter_type_declaration) x
  | Psig_typext(x) -> iter_type_extension x
...
and iter_structure_item_desc c1 =
  ...
  | Pstr_type(x) -> (iter_list iter_type_declaration) x
  | Pstr_typext(x) -> iter_type_extension x
