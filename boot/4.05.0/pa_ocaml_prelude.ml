...
module Initial =
  struct
    ...
    let signature = Earley_parser.Earley.declare_grammar "signature" 
    let _ =
      Earley_parser.Earley.set_grammar signature
        (Earley_parser.Earley.fsequence
           (Earley_parser.Earley.apply (fun f  -> f [])
              (Earley_parser.Earley.fixpoint' (fun l  -> l) signature_item
                 (fun x  -> fun f  -> fun l  -> f (x :: l))))
           (Earley_parser.Earley.empty (fun l  -> List.flatten l)))
    ...
    let union_re l =
      ...
      String.concat "\\|" l 
    let arrow_re = Earley_parser.Earley.declare_grammar "arrow_re" 
    let _ =
      Earley_parser.Earley.set_grammar arrow_re
    ...
    let (infix_symbol,infix_symbol__set__grammar) =
      Earley_parser.Earley.grammar_family "infix_symbol" 
    let _ =
               ...
               then
                 [Earley_parser.Earley.fsequence
                    (Earley_str.regexp (infix_symb_re prio)
                       (fun groupe  -> groupe 0))
                    (Earley_parser.Earley.fsequence not_special
    ...
    let (prefix_symbol,prefix_symbol__set__grammar) =
      Earley_parser.Earley.grammar_family "prefix_symbol" 
    let _ =
    ...
    let mutable_flag = Earley_parser.Earley.declare_grammar "mutable_flag" 
    let _ =
      Earley_parser.Earley.set_grammar mutable_flag
        (Earley_parser.Earley.alternatives
           [Earley_parser.Earley.fsequence_ignore
              (Earley_parser.Earley.empty ())
              (Earley_parser.Earley.empty Immutable);
           Earley_parser.Earley.fsequence mutable_kw
             (Earley_parser.Earley.empty (fun _default_0  -> Mutable))])
      
    let private_flag = Earley_parser.Earley.declare_grammar "private_flag" 
    let _ =
      Earley_parser.Earley.set_grammar private_flag
        (Earley_parser.Earley.alternatives
           [Earley_parser.Earley.fsequence_ignore
              (Earley_parser.Earley.empty ())
              (Earley_parser.Earley.empty Public);
           Earley_parser.Earley.fsequence private_kw
             (Earley_parser.Earley.empty (fun _default_0  -> Private))])
      
    let virtual_flag = Earley_parser.Earley.declare_grammar "virtual_flag" 
    let _ =
      Earley_parser.Earley.set_grammar virtual_flag
        (Earley_parser.Earley.alternatives
           [Earley_parser.Earley.fsequence_ignore
              (Earley_parser.Earley.empty ())
              (Earley_parser.Earley.empty Concrete);
           Earley_parser.Earley.fsequence virtual_kw
             (Earley_parser.Earley.empty (fun _default_0  -> Virtual))])
      
    let rec_flag = Earley_parser.Earley.declare_grammar "rec_flag" 
    let _ =
      Earley_parser.Earley.set_grammar rec_flag
        (Earley_parser.Earley.alternatives
           [Earley_parser.Earley.fsequence_ignore
              (Earley_parser.Earley.empty ())
              (Earley_parser.Earley.empty Nonrecursive);
           Earley_parser.Earley.fsequence rec_kw
             (Earley_parser.Earley.empty (fun _default_0  -> Recursive))])
      
    let downto_flag = Earley_parser.Earley.declare_grammar "downto_flag" 
    let _ =
      Earley_parser.Earley.set_grammar downto_flag
        (Earley_parser.Earley.alternatives
           [Earley_parser.Earley.fsequence downto_kw
              (Earley_parser.Earley.empty (fun _default_0  -> Downto));
           Earley_parser.Earley.fsequence to_kw
             (Earley_parser.Earley.empty (fun _default_0  -> Upto))])
